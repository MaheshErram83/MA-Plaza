...
export async function sendMessage(houseId, memberId, text) {
  const id = uid();
  await run("INSERT INTO messages (id, house_id, member_id, text, created_at) VALUES ($1, $2, $3, $4, $5)",
    [id, houseId, memberId, text, Date.now()]);
  return id;
}
