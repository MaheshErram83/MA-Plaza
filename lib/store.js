...
export async function initDB() {
  await run(`
    ...
    CREATE TABLE IF NOT EXISTS contributions (
      ...
      amount REAL NOT NULL, month_tag TEXT NOT NULL, created_at BIGINT NOT NULL
    );
    ...
    CREATE TABLE IF NOT EXISTS reimbursements (
      ...
      treasurer_note TEXT, created_at BIGINT NOT NULL, updated_at BIGINT NOT NULL
    );
  `);
}
...
// ---- Contributions ----
export async function addContribution(houseId, memberId, amount) {
  const mt = currentMonthTag();
  await run("INSERT INTO contributions (id, house_id, member_id, amount, month_tag, created_at) VALUES ($1, $2, $3, $4, $5, $6)",
    [uid(), houseId, memberId, amount, mt, Date.now()]);
  const n = await nameOf(houseId, memberId);
  await addFeed(houseId, "contribution", n + " contributed ₹" + Math.round(amount).toLocaleString("en-IN"), "success");
}
...
export async function advanceReimbursement(houseId, id, toStatus, treasurerNote) {
